# -*- coding: utf-8 -*-
"""
Created on Fri Oct 28 07:35:48 2022
...
def loading_registers(key): #loads registers using a 23-bit key as a parameter
    i = 0
    while(i < reg_x_length): 
        reg_x.insert(i, int(key[i])) #takes first 6 elements from key
        i = i + 1
    j = 0
    p = reg_x_length
    while(j < reg_y_length): 
        reg_y.insert(j,int(key[p])) #takes next 8 elements from key
        p = p + 1
        j = j + 1
    k = reg_y_length + reg_x_length
    r = 0
    while(r < reg_z_length): 
        reg_z.insert(r,int(key[k])) #takes next 9 elements from key
        k = k + 1
        r = r + 1
        
def set_key(key): #sets the key and loads the registers if it contains 0's and 1's and if it's exactly 23 bits 
    if(len(key) == 23 and re.match("^([01])+", key)):
        key_one = key
...
def get_keystream(length): #calculates the keystream by XOR-ing the appropriate indeces
    ...
    while i < length:
        ...
        majority = get_majority(reg_x_temp[1], reg_y_temp[3], reg_z_temp[3])
        if reg_x_temp[1] == majority: 
            new = reg_x_temp[2] ^ reg_x_temp[4] ^ reg_x_temp[5]
        ...
        if reg_y_temp[3] == majority:
            new_one = reg_y_temp[6] ^ reg_y_temp[7]
        ...
        if reg_z_temp[3] == majority:
            new_two = reg_z_temp[2] ^ reg_z_temp[7] ^ reg_z_temp[8]
        ...
        keystream.insert(i, reg_x_temp[5] ^ reg_y_temp[7] ^ reg_z_temp[8])
        i = i + 1
        return keystream
...
def user_input_key(): #input the key from the console
	tha_key = str(input('Enter a 23-bit key: '))
	if (len(tha_key) == 64 and re.match("^([01])+", tha_key)):
		return tha_key
	else:
		while(len(tha_key) != 23 and not re.match("^([01])+", tha_key)):
			if (len(tha_key) == 23 and re.match("^([01])+", tha_key)):
				return tha_key
...
#Example of 64-bit key: 10001011011010111001101
tha_main()